...
Component({
  ...
  methods: {
    ...
    createEmptyGrids(year, month) {
      ...
      let thisMonthDays = this.getThisMonthDays(year, month)//当夜天数
      let preMonthDays = month - 1 < 0 ? this.getThisMonthDays(year - 1, 12) : this.getThisMonthDays(year - 1, month)
      for (let i = 1; i <= emptyDays; i++) {
        emptyGridsBefore.push(
          preMonthDays - (emptyDays - i)
        )
      }
      let after = (42 - thisMonthDays - emptyDays) - 7 >= 0 ? (42 - thisMonthDays - emptyDays) - 7
        : (42 - thisMonthDays - emptyDays)
      for (let i = 1; i <= after; i++) {
        emptyGridsAfter.push(
          i)
      }
      this.setData({
        emptyGridsAfter,
